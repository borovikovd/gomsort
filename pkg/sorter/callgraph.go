	...
	for _, decl := range file.Decls {
		if funcDecl, ok := decl.(*ast.FuncDecl); ok {
			if method := extractMethodInfo(funcDecl); method != nil && funcDecl.Body != nil {
				visitor := &callVisitor{callGraph: cg, currentReceiver: method.ReceiverName, currentMethod: method.Name}
	...
	case *ast.CallExpr:
		if sel, ok := n.Fun.(*ast.SelectorExpr); ok {
			if ident, ok := sel.X.(*ast.Ident); ok {
				if ident.Name == "self" || ident.Name == v.currentReceiver || (len(ident.Name) == 1 && len(v.currentReceiver) > 0 && strings.EqualFold(ident.Name[0:1], v.currentReceiver[0:1])) {
